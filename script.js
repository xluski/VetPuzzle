...
(async () => {
  ...
  function getIds() {
    ...
    return {
      assignmentId: location.pathname.match(/assignments\/([a-z0-9]+)/i)?.[1] || null,
      attachmentId: params.get("attachmentId") || null,
    };
  }
  ...
  async function fetchEdpuzzleDirect() {
    ...
    if (!r.ok) throw new Error(`Edpuzzle API returned ${r.status}`);
    return r.json();
  }
    ...
    for (const q of qs) {
      ...
      if (q.type === "multiple-choice" || q.type === "mc") {
        ...
        questions.push({
          ...
          question: text,
          answers: correct.map(
            (c) => c.body?.find?.((b) => b.text)?.text || c.body?.[0]?.text || c.text || ""
          ),
          time: q.time,
          id: q._id,
        });
  ...
  function skipVideo() {
    ...
    } catch (e) { console.warn("[VetPuzzle] skipVideo failed:", e); }
  }
  ...
  // ── BUILD POPUP UI ────────────────────────────────────────────
  function buildUI(questions, title) {
...
<div class="assign-card">
  ...
  <div class="assign-title">${title || "Untitled Assignment"}</div>
...
function runAll(){const speed=parseFloat(document.getElementById('spd').value);window.opener?.postMessage({type:'vetpuzzle-run',state,speed},'*');toast('Running...');}
...
<\/script>
  ...
  async function main() {
    if (!location.href.includes("edpuzzle.com")) {
      alert("[VetPuzzle] Make sure you're on an edpuzzle.com assignment page.");
      return;
    }
